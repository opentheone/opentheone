...
- **多模型支持**：兼容所有 OpenAI Chat 协议的模型（DeepSeek、OpenAI、Qwen、Kimi…），内置 5 个常用 provider 预置一键填表。
- **长期记忆**：四层金字塔（L0 消息 → L1 原子记忆 → L2 主题场景 → L3 用户画像），全程 LLM 主导 + BM25 关键词检索，**无需 embedding 模型**。
...
每次发布会由 CI 把多架构镜像（linux/amd64 + linux/arm64）推到 GitHub Container Registry，直接拉来用就行，**不用本地构建**。
...
docker run -d \
  ...
  -p 8080:8080 \
  -v oto-data:/app/data \
  ghcr.io/opentheone/opentheone:latest
```

如果想要 healthcheck、env 注入、便于升级，用我们提供的 compose 文件：

```bash
curl -fsSL https://raw.githubusercontent.com/opentheone/opentheone/main/docker-compose.yml -o docker-compose.yml
docker compose up -d
```
...
- `ghcr.io/opentheone/opentheone:latest` — 跟随最新 release
- `ghcr.io/opentheone/opentheone:v0.0.3`（或 `0.0.3` / `0.0` / `0`）— 版本固定，推荐生产环境
- 想跑某个具体版本，给 compose 传 `OTO_IMAGE=ghcr.io/opentheone/opentheone:v0.0.3 docker compose up -d` 即可。

升级：`docker compose pull && docker compose up -d`（或者 `docker pull ghcr.io/opentheone/opentheone:latest && docker restart opentheone`）。
...
```bash
# 终端 A：后端（必须带 sqlite_fts5 build tag，否则启动时 FTS5 模块缺失会报错）
make dev
# 或：cd backend && go run -tags sqlite_fts5 ./cmd/server

# 终端 B：前端（Vite 已经把 /api 代理到 :8080）
cd frontend && pnpm dev
```

> 长期记忆走 SQLite FTS5，必须以 `-tags sqlite_fts5` 编译 / 运行。
> `make build` / `make dev` / `make test` / 官方 Dockerfile 都已带上这个 tag，
> 直接用 `go build` / `go run` / `go test` 时需要自己加。
...
- 多媒体生成（让 TA 主动发图、发语音）。
- 真正的中文分词（jieba via cgo）替换当前的 bigram，进一步提升 BM25 召回精度。
